...
public class Midboss2 : EnemyBase
{
    int shotCounter = 0;
    int mode = 0;
    int impatience = 1200;
    readonly int startShootingAt;
    readonly SFPoint[] gatlingOffsets = new SFPoint[] { new SFPoint((sfloat)0, (sfloat)(-5)), new SFPoint((sfloat)5, (sfloat)0), new SFPoint((sfloat)0, (sfloat)5), new SFPoint((sfloat)(-5), (sfloat)0) };
    public Midboss2(GameManager gManager, int drawOrder = 0) : base(gManager, drawOrder)
    {
    ...
    public override List<BulletInfo> Shoot(GameManager gManager, EnemyManager eManager, BulletManager bManager, PlayerController player)
    {
        ...
        if (shootable)
        {
            ...
            {
                case 0:
                    if (shotCounter >= startShootingAt + spawnRank / 100000 + 10)
                    {
                    ...
                    if (shotCounter >= startShootingAt)
                    {
                        ...
                        else
                        {
                            shotSealed[0] = -1;
                            SFPoint shotDir;
                            shotDir = (player.GetSFPosition() - (position + shotSealPoints[0] + gatlingOffsets[(shotCounter - startShootingAt) % 4])).Normalized();
                            rtnValue.Add(new BulletInfo(
                                new Bullet(BulletPack.BltColor.Blue, BulletPack.BltShape.Bean, position + shotSealPoints[0] + gatlingOffsets[(shotCounter - startShootingAt) % 4], shotDir * ((sfloat)3 + (sfloat)(shotCounter - startShootingAt) * (sfloat)0.5))
                            ));
                            bManager.CreateFlash(new FlashFX(position + shotSealPoints[0] + gatlingOffsets[(shotCounter - startShootingAt) % 4], FlashType.Circle8px, (sfloat)0));
                        }
                    }
                    break;
                case 1:
                    if (shotCounter >= startShootingAt + 50)
                    {
                        //if (shotSealed[0] < 0) shotSealed[0] = 0;
                        shotCounter = 0;
                        mode++;
                    ...
                    if (shotCounter >= startShootingAt && (shotCounter - startShootingAt) % 10 == 0)
                    {
                        ...
                        else
                        {
                            //shotSealed[0] = -1;
                            SFPoint[] shotDirs = {
                            (player.GetSFPosition() - ( position + shotSealPoints[0] + gatlingOffsets[0] )).Normalized(),
                            (player.GetSFPosition() - ( position + shotSealPoints[0] + gatlingOffsets[1] )).Normalized(),
                            (player.GetSFPosition() - ( position + shotSealPoints[0] + gatlingOffsets[2] )).Normalized(),
                            (player.GetSFPosition() - ( position + shotSealPoints[0] + gatlingOffsets[3] )).Normalized()
                        };
                            switch (shotCounter - startShootingAt)
                            {
                                case 10:
                                    shotDirs[0] = shotDirs[0].Rotated((sfloat)(-0.25));
                                    shotDirs[1] = shotDirs[1].Rotated((sfloat)(-0.25));
                                    shotDirs[2] = shotDirs[2].Rotated((sfloat)(0.25));
                                    shotDirs[3] = shotDirs[3].Rotated((sfloat)(0.25));
                                    break;
                                case 30:
                                    shotDirs[0] = shotDirs[0].Rotated((sfloat)(0.25));
                                    shotDirs[1] = shotDirs[1].Rotated((sfloat)(0.25));
                                    ...
                                    break;
                                default:
                                    shotDirs[1] = shotDirs[1].Rotated((sfloat)(-0.5));
                                    shotDirs[3] = shotDirs[3].Rotated((sfloat)(0.5));
                                    break;
                            }
                            int pos = 0;
                            foreach (SFPoint shootTo in shotDirs)
                            {
                                rtnValue.Add(new BulletInfo(
                                             new Bullet(BulletPack.BltColor.Red, BulletPack.BltShape.Bean, position + shotSealPoints[0] + gatlingOffsets[pos], shootTo * ((sfloat)spawnRank / (sfloat)400000 + (sfloat)2.5))
                                             ));
                                bManager.CreateFlash(new FlashFX(position + shotSealPoints[0] + gatlingOffsets[(shotCounter - startShootingAt) % 4], FlashType.Circle8px, (sfloat)0));
                                pos++;
                            }
                ...
                default:
                    if (shotCounter >= startShootingAt)
                    {
                        if (shotSealed[0] > 0)
                        {
                            gManager.rank += 10000 * (spawnRank / 100000 + 1);
                            gManager.rank = Math.Min(gManager.rank, 999999);
                        }
                        else
                        {
                            ...
                            sfloat rotation = position.x - player.GetSFPosition().x >= (sfloat)0 ? (sfloat)0.35 : (sfloat)(-0.35);
                            for (int i = 0; i < spawnRank / 100000 + 1; i++)
                            {
                                shotDir = new SFPoint(libm.sinf((sfloat)i * rotation * sfloat.FromRaw(0x40490fdb)), libm.cosf((sfloat)i * rotation * sfloat.FromRaw(0x40490fdb)));
                                directs = Round(shotDir, 10);
                                foreach (SFPoint shootTo in directs)
                                    foreach (SFPoint offset in gatlingOffsets) { 
                                        rtnValue.Add(new BulletInfo(
                                                        new Bullet(BulletPack.BltColor.Purple, BulletPack.BltShape.Bean, position + shotSealPoints[0] + offset, shootTo * ((sfloat)spawnRank / (sfloat)400000 + (sfloat)2 + (sfloat)i * (sfloat)0.5))
                                        ));
                                        bManager.CreateFlash(new FlashFX(position + shotSealPoints[0] + offset, FlashType.Circle8px, (sfloat)0));
                                    }
    ...
    public override List<DrawInfo> GetDrawFrames(GameManager gManager, PlayerController player, SpriteFrames frames)
    {
        ...
        if (lifeTime % 2 == 0)
        {
            rtnValue.Add(new DrawInfo(frames.GetFrame("Midbosses", 2), Vector2.Up * 26 + Vector2.Left * 26, 0, Vector2.One));
            rtnValue.Add(new DrawInfo(frames.GetFrame("Midbosses", 2), Vector2.Up * 26 + Vector2.Left * -26, 0, Vector2.One));
        }
        rtnValue.Add(new DrawInfo(frames.GetFrame("Midbosses", 1), Vector2.Zero, 0, Vector2.One, damagedOnThisFrame ? new Color(0, 1, 1) : new Color(1, 1, 1)));
    ...
    public override List<ExplosionFX> Explosions()
    {
        ...
        {
            rtnValue.Add(new ExplosionFX(position, direct, (sfloat)0.9, Math.Abs(timer) / 3, 2));
            timer -= 3;
        }
