...
public class Aimed : EnemyBase
{
    int shotCounter = 0;
    public Aimed(GameManager gManager, PlayerController player, SFPoint spawnpoint, int drawOrder = 0) : base(gManager, drawOrder)
    {
        position = spawnpoint;
        hitboxGroup = new SFAABB[] { new SFAABB(new SFPoint((sfloat)0, (sfloat)0), new SFPoint((sfloat)10, (sfloat)10)) };
        movement = (player.GetSFPosition() - position).Normalized() * ((sfloat)spawnRank / (sfloat)200000 + (sfloat)2);
        hp = 1;
        baseValue = 150;
